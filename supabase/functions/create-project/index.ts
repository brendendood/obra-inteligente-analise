...
async function getUserContext(
  ...
  // Get user's current plan_code from users table
  const { data: userData, error: userError } = await serverSupabase
    .from('users')
    .select('plan_code, lifetime_base_consumed')
    .eq('id', userId)
    .single();
  ...
  const planCode = (userData.plan_code as 'BASIC' | 'PRO' | 'ENTERPRISE') || 'BASIC';
  
  // Define base quota based on plan_code (Lovable/Stripe integration)
  const baseQuota = planCode === 'BASIC' ? 7 : 
                    planCode === 'PRO' ? 20 : 
                    planCode === 'ENTERPRISE' ? null : 7; // null = unlimited

  return {
    userId,
    planCode,
    baseQuota,
    lifetimeBaseConsumed: userData.lifetime_base_consumed || 0
  };
}
