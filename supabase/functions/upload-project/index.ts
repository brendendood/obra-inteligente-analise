...
serve(async (req) => {
  ...
  try {
    console.log('🚀 Upload project function called - ENHANCED VERSION')
    ...
    console.log('🔄 Generating budget and schedule data...')
    ...
    console.log('✅ Budget generated:', {
      totalCost: budgetData.total_cost,
    ...
    const analysisData = {
      ...
      // DADOS PERSISTIDOS DO ORÇAMENTO
      budget_data: budgetData,
      // DADOS PERSISTIDOS DO CRONOGRAMA  
    ...
    console.log('💾 Persisting project with complete analysis data...')
    ...
    const { data: project, error: projectError } = await supabase
      ...
      .insert({
        ...
        estimated_budget: budgetData.total_cost,
    ...
    console.log('🎉 Project created successfully with complete data:', project.id)

    const message = isRealProject 
      ? `Projeto arquitetônico analisado! Orçamento de R$ ${budgetData.total_cost.toLocaleString('pt-BR')} e cronograma de ${scheduleData.total_duration} dias gerados automaticamente.`
      : `PDF processado! Documento analisado com dados estimados para ${estimatedArea}m².`
