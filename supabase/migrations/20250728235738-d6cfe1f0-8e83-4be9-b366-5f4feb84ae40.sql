-- Corrigir função de atualização em massa removendo MAX(uuid) que não é suportado
DROP FUNCTION IF EXISTS public.force_update_all_active_users_geolocation();
...
BEGIN
    ...
    FOR user_record IN 
        SELECT DISTINCT ON (ulh.user_id, ulh.ip_address::text)
            ulh.user_id, 
            ulh.ip_address::text as ip_text,
            ulh.login_at as last_login,
            ulh.id as latest_login_id
        ...
        ORDER BY ulh.user_id, ulh.ip_address::text, ulh.login_at DESC
    LOOP
