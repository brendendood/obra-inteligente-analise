-- Limpeza completa de usuários - versão com aliases
...
BEGIN
    ...
    RAISE NOTICE 'Admin user_id encontrado: %', target_admin_id;
    
    -- FASE 0: Deletar permissões admin que referenciam outros usuários
    DELETE FROM admin_permissions WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Permissões admin deletadas: %', deleted_count;
    
    -- FASE 1: Deletar dados de projetos (cascata automática)
    DELETE FROM projects WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Projetos deletados: %', deleted_count;
    
    -- FASE 2: Deletar conversas AI
    DELETE FROM ai_conversations WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Conversas AI deletadas: %', deleted_count;
    
    -- FASE 3: Deletar mensagens AI órfãs (se houver)
    DELETE FROM ai_messages WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Mensagens AI deletadas: %', deleted_count;
    
    -- FASE 4: Deletar histórico de chat N8N
    DELETE FROM n8n_chat_histories WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Histórico N8N deletado: %', deleted_count;
    
    -- FASE 5: Deletar dados de gamificação
    DELETE FROM gamification_logs WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Logs de gamificação deletados: %', deleted_count;
    
    DELETE FROM user_gamification WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Dados de gamificação deletados: %', deleted_count;
    
    -- FASE 6: Deletar histórico de login
    DELETE FROM user_login_history WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Histórico de login deletado: %', deleted_count;
    
    -- FASE 7: Deletar analytics
    DELETE FROM user_analytics WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Analytics deletados: %', deleted_count;
    
    DELETE FROM user_analytics_enhanced WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Analytics enhanced deletados: %', deleted_count;
    
    -- FASE 8: Deletar pagamentos
    DELETE FROM user_payments WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Pagamentos deletados: %', deleted_count;
    
    DELETE FROM payments WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Registros de pagamento deletados: %', deleted_count;
    
    -- FASE 9: Deletar logs de acesso ao chat
    DELETE FROM chat_access_logs WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Logs de acesso ao chat deletados: %', deleted_count;
    
    -- FASE 10: Deletar métricas de AI
    DELETE FROM ai_usage_metrics WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Métricas de AI deletadas: %', deleted_count;
    
    -- FASE 11: Deletar segmentos de usuário
    DELETE FROM user_segments WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Segmentos de usuário deletados: %', deleted_count;
    
    -- FASE 12: Deletar documentos de projeto (se houver)
    DELETE FROM project_documents WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Documentos de projeto deletados: %', deleted_count;
    
    -- FASE 13: Deletar logs de impersonação (manter apenas do admin)
    DELETE FROM admin_impersonation_logs WHERE admin_id != target_admin_id AND user_impersonated_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Logs de impersonação deletados: %', deleted_count;
    
    -- FASE 14: Deletar logs de segurança admin
    DELETE FROM admin_security_logs WHERE admin_id != target_admin_id AND (target_user_id != target_admin_id OR target_user_id IS NULL);
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Logs de segurança admin deletados: %', deleted_count;
    
    -- FASE 15: Deletar logs de auditoria admin usando alias
    DELETE FROM admin_audit_logs aal WHERE aal.admin_user_id != target_admin_id AND (aal.target_id != target_admin_id OR aal.target_id IS NULL);
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Logs de auditoria admin deletados: %', deleted_count;
    
    -- FASE 16: Deletar assinaturas de usuários
    DELETE FROM user_subscriptions WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Assinaturas deletadas: %', deleted_count;
    
    -- FASE 17: Deletar perfis de usuários
    DELETE FROM user_profiles WHERE user_id != target_admin_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Perfis de usuário deletados: %', deleted_count;
    
    -- FASE 18: Deletar usuários da auth (cascata final)
