-- Limpeza completa de usuários - versão corrigida
...
BEGIN
    ...
    DELETE FROM admin_permissions WHERE user_id != admin_user_id AND granted_by != admin_user_id;
    ...
    DELETE FROM admin_security_logs WHERE admin_id != admin_user_id AND target_user_id != admin_user_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Logs de segurança admin deletados: %', deleted_count;
    
    -- FASE 15: Deletar logs de auditoria admin
    DELETE FROM admin_audit_logs WHERE admin_user_id != admin_user_id AND target_id != admin_user_id;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
