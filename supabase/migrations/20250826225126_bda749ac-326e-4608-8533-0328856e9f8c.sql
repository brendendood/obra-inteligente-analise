-- Complete fix for all Security Definer View issues
-- The issue might be that views are still calling functions that have SECURITY DEFINER

-- First, let's check what views exist and their definitions
DO $$
DECLARE
    view_record RECORD;
    view_def TEXT;
BEGIN
    -- Log current views for debugging
    FOR view_record IN 
...
-- Recreate safe_admin_users view WITHOUT using security definer functions
-- Instead, rely on proper RLS policies on the base table
CREATE VIEW public.safe_admin_users AS
SELECT 
    id,
    email,
    full_name,
...
DROP POLICY IF EXISTS "Superusers can view admin users" ON public.admin_users;
DROP POLICY IF EXISTS "Only superusers can manage admin users" ON public.admin_users;
DROP POLICY IF EXISTS "Only superusers can view admin users" ON public.admin_users;

-- Create proper RLS policy that doesn't use security definer functions
CREATE POLICY "Only admin users can view admin users" ON public.admin_users
FOR SELECT USING (
    EXISTS (
...
-- Recreate v_crm_client_stats view with explicit owner filtering
CREATE VIEW public.v_crm_client_stats AS
SELECT 
    c.id AS client_id,
...
-- Recreate v_project_summary view  
