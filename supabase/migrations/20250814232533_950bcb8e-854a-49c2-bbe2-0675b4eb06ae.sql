-- Limpeza completa de usuários - versão final corrigida
...
BEGIN
    ...
    DELETE FROM admin_permissions WHERE user_id != admin_user_id;
    ...
    DELETE FROM admin_security_logs WHERE admin_security_logs.admin_id != admin_user_id AND (admin_security_logs.target_user_id != admin_user_id OR admin_security_logs.target_user_id IS NULL);
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE 'Logs de segurança admin deletados: %', deleted_count;
    
    -- FASE 15: Deletar logs de auditoria admin (qualificar nomes de tabela)
    DELETE FROM admin_audit_logs WHERE admin_audit_logs.admin_user_id != admin_user_id AND (admin_audit_logs.target_id != admin_user_id OR admin_audit_logs.target_id IS NULL);
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
