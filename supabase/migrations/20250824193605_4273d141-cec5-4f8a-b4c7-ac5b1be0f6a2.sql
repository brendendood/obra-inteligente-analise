...
-- 1) Criar novo ENUM se necessário
DO $$
BEGIN
...
-- 2) Migrar tabela user_plans existente
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='user_plans') THEN
    -- Migrar valores antigos para novo enum
    ALTER TABLE public.user_plans ADD COLUMN IF NOT EXISTS plan_tier_new public.plan_tier_v2;
    ...
    END
    WHERE plan_tier_new IS NULL;
    
    -- Substituir coluna antiga
...
-- 4) Tabela de tracking de mensagens IA
...
-- RLS para ai_message_usage
CREATE POLICY IF NOT EXISTS "select_own_ai_usage"
  ON public.ai_message_usage
  FOR SELECT
  TO authenticated
  USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "insert_own_ai_usage"
  ON public.ai_message_usage
  FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "update_own_ai_usage"
  ON public.ai_message_usage
  FOR UPDATE
  TO authenticated
  USING (auth.uid() = user_id)
...
-- 5) Backfill para usuários sem plano
