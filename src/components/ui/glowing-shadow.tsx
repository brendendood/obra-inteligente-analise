  ...
  return (
    <>
      <style>{`
        ...
        /* ADAPTATIVO AOS TEMAS: usa tokens do Tailwind/shadcn */
        .glow-container {
          --card-color: hsl(var(--background));
          --text-color: hsl(var(--foreground));
          --card-radius: 9999px;
          --card-width: 22rem; /* largura base; responsivo via container */
          --border-width: 3px;
          --bg-size: 1;
          --hue: 0; --hue-speed: 1; --rotate: 0;
          --animation-speed: 4s; --interaction-speed: 0.55s;
          --glow-scale: 1.5; --scale-factor: 1;
          --glow-blur: 6; --glow-opacity: 1;
          --glow-radius: 100; --glow-rotate-unit: 1deg;
        ...
        .glow { --glow-translate-y: 0; position: absolute; width: 5rem; height: 5rem;
          animation: rotate var(--animation-speed) linear infinite;
          transform: rotateZ(calc(var(--rotate) * var(--glow-rotate-unit)));
          transform-origin: center; border-radius: calc(var(--glow-radius) * 10vw);
        }
        .glow:after {
          content: ""; z-index: -2; filter: blur(calc(var(--glow-blur) * 10px));
          width: 130%; height: 130%; left: -15%; top: -15%;
          background: hsl(calc(var(--hue) * var(--hue-speed) * 1deg) 100% 60%);
          position: relative; border-radius: calc(var(--glow-radius) * 10vw);
          animation: hue-animation var(--animation-speed) linear infinite;
