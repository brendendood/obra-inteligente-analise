...
import { Bot, Home, Users2, User, CreditCard, CircleHelp, MessageCircle, Gift, LogOut } from "lucide-react";
...
export function SessionNavBar() {
  ...
  return <motion.aside role="navigation" aria-label="Menu lateral do usuário" className={cn("relative h-screen shrink-0 border-r border-border/30", "bg-background text-foreground", "flex flex-col")} initial={isCollapsed ? "closed" : "open"} animate={isCollapsed ? "closed" : "open"} variants={sidebarVariants} onMouseEnter={() => setIsCollapsed(false)} onMouseLeave={() => setIsCollapsed(true)}>
      ...
      <ScrollArea className="flex-1">
        <nav className="p-2">
          <ul className="flex flex-col gap-1">
            {items.map(item => {
            const active = typeof item.activeMatch === "function" ? item.activeMatch(location.pathname ?? "") : location.pathname === item.href;
            return <li key={item.label}>
                  <Link to={item.href} className={cn("group flex items-center gap-3 rounded-md px-2 py-2 transition-colors", "text-muted-foreground hover:text-foreground", "hover:bg-accent", active && "bg-accent text-foreground")} aria-current={active ? "page" : undefined}>
                    <span className="grid place-items-center w-5 h-5 flex-shrink-0">
                      {item.icon}
                    </span>

                    {/* Rótulo que some quando colapsado */}
                    <motion.span className="text-sm truncate" variants={labelVariants} animate={isCollapsed ? "closed" : "open"}>
                      {item.label}
                    </motion.span>
                  </Link>
                </li>;
            ...
            <div className="flex items-center gap-3">
              <Avatar className="h-8 w-8">
                <AvatarFallback className="bg-primary text-primary-foreground text-xs">
                  {user?.user_metadata?.full_name?.split(' ').map((n: string) => n[0]).join('').slice(0, 2) || user?.email?.slice(0, 2).toUpperCase() || 'U'}
                </AvatarFallback>
              </Avatar>
